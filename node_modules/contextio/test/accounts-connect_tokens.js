...
vows.describe('ContextIO/accounts/connect_tokens').addBatch({
	'A list': {
		'being fetched': {
			topic: function () {
				ctxio.accounts(tData.account_id).connect_tokens().get(this.callback);
			},
	...
	'Manipulating instances ': {
		'by creating one': {
			topic: function () {
				ctxio.accounts(tData.account_id).connect_tokens().post(tDataBase.createParams, this.callback);
			},
			...
			'and fetching it': {
				topic: function (createResp) {
					ctxio.accounts(tData.account_id).connect_tokens(createResp.body.token).get(this.callback);
				},
				'made the right API call': hlp.macros.assertCall('accounts/'+tData.account_id+'/connect_tokens/*', 'GET'),
				'responds with 200': hlp.macros.assertStatus(200),
				...
				'then deleting it': {
					topic: function (fetchResp) {
						ctxio.accounts(tData.account_id).connect_tokens(fetchResp.body.token).delete(this.callback);
					},
					...
					'and attempting to fetch it': {
						topic: function (deleteResp, deleteReqInfo, fetchResp) {
							ctxio.accounts(tData.account_id).connect_tokens(fetchResp.body.token).get(this.callback);
						},
						'made the right API call': hlp.macros.assertCall('accounts/'+tData.account_id+'/connect_tokens/*', 'GET'),
