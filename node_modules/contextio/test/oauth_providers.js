...
vows.describe('ContextIO/oauth_providers').addBatch({
	'Listing instances ': {
		topic: function () {
			ctxio.oauth_providers().get(this.callback);
		},
	...
	'Manipulating instances ': {
		'by creating one': {
			topic: function () {
				ctxio.oauth_providers().post(tData.createParams, this.callback);
			},
			...
			'and fetching it': {
				topic: function (createResp) {
					ctxio.oauth_providers(createResp.body.provider_consumer_key).get(this.callback);
				},
				'made the right API call': hlp.macros.assertCall('oauth_providers/*', 'GET'),
				'responds with 200': hlp.macros.assertStatus(200),
				...
				'then deleting it': {
					topic: function (fetchResp) {
						ctxio.oauth_providers(fetchResp.body.provider_consumer_key).delete(this.callback);
					},
					...
					'and attempting to fetch it': {
						topic: function (deleteResp, deleteReqInfo, fetchResp) {
							ctxio.oauth_providers(fetchResp.body.provider_consumer_key).get(this.callback);
						},
