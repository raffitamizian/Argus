...
vows.describe('ContextIO/accounts/email_addresses').addBatch({
	'A list': {
		'being fetched': {
			topic: function () {
				ctxio.accounts(tData.account_id).email_addresses().get(this.callback);
			},
	...
	'Manipulating the resource': {
		'by creating an instance': {
			topic: function () {
				ctxio.accounts(tData.account_id).email_addresses().post(tData.createParams, this.callback);
			},
			...
			'and fetching the list': {
				...
				'then deleting it': {
					topic: function () {
						ctxio.accounts(tData.account_id).email_addresses(tData.createParams.email_address).delete(this.callback);
					},
	...
	'Changing the primary email': {
		topic: function () {
			ctxio.accounts(tData.account_id).email_addresses(tData.existing[1]).post({primary:1}, this.callback);
		},
		...
		'and fetching the list again': {
			...
			'settings the primary back to expected': {
				topic: function () {
					ctxio.accounts(tData.account_id).email_addresses(tData.existing[0]).post({primary:1}, this.callback);
				},
