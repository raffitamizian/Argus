...
vows.describe('ContextIO/connect_tokens').addBatch({
	'A list': {
		'being fetched': {
			topic: function () {
				ctxio.connect_tokens().get(this.callback);
			},
	...
	'An instance': {
		'being fetched': {
			topic: function () {
				ctxio.connect_tokens(tData.token).get(this.callback);
			},
	...
	'Manipulating instances ': {
		'by creating one': {
			topic: function () {
				ctxio.connect_tokens().post(tData.createParams, this.callback);
			},
			...
			'and fetching it': {
				topic: function (createResp) {
					ctxio.connect_tokens(createResp.body.token).get(this.callback);
				},
				'made the right API call': hlp.macros.assertCall('connect_tokens/*', 'GET'),
				'responds with 200': hlp.macros.assertStatus(200),
				...
				'then deleting it': {
					topic: function (fetchResp) {
						ctxio.connect_tokens(fetchResp.body.token).delete(this.callback);
					},
					...
					'and attempting to fetch it': {
						topic: function (deleteResp, deleteReqInfo, fetchResp) {
							ctxio.connect_tokens(fetchResp.body.token).get(this.callback);
						},
